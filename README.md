...
- GD_Controller.h/.cpp:
    ...
    - **核心**:
        ...
        2. **事件处理**: 通过onPlayerPlay、onPlayerPass等槽函数接收来自视图的玩家操作。
            
        3. **逻辑调度**: 调用模型类（如Player, CardCombo）来处理业务逻辑。例如，验证出牌是否合法、从玩家手牌中移除卡牌、更新桌面牌型。
            
        4. **视图更新**: 在模型状态改变后，通过发射sig...信号（如sigUpdatePlayerHand, sigUpdateTableCards）来通知视图进行界面刷新。
